...
このアプリではサンタさんの「プレゼント配送計画データ」を分析します。配送計画表には、どの国に何個、何時にプレゼントを配達予定と記されています。  
このデータを分かりやすく読み解くことがこのアプリの目的ですが、まずはデータをStreamlitアプリに取り込むところまでを実装しました。
...
StreamlitとPandasを使用した分析はこのあと数回続きます！お楽しみに！
